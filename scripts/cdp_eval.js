/* Minimal CDP driver: serve the workspace, open a URL in headless Chromium,
 * wait for load, evaluate a JS expression, print its JSON value.
 *
 * Usage: node cdp_eval.js <url> <expression> [settleMs]
 * Example: node cdp_eval.js http://127.0.0.1:8125/mp_engine_test.html 'JSON.stringify(window.__ET)'
 */
'use strict';
...
(async () => {
  ...
  const settle = parseInt(process.argv[4] || '800', 10);
  if (!url || !expr) { console.error('usage: node cdp_eval.js <url> <expression> [settleMs]'); process.exit(2); }
  ...
  // Resolve once the page fires loadEventFired.
  ...
  process.exit(0);
})().catch((e) => { console.error('[cdp_eval] fatal: ' + (e && e.stack || e)); process.exit(2); });
