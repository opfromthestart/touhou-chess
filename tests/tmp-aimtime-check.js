...
// pocket). The key claim: with aimTime:1.6 the gap must anchor at the player's
// t=1.6 position (dead below the boss); without aimTime it anchors at the
// t=2.6 position (player has drifted left).
//
...
// Run until the fan first fires (t >= 2.6), returning a snapshot of the fan
// bullets' (vx, vy) at that instant. moveFn scripts the player each frame.
function fanAnglesAtFire(e, emitDef, moveFn) {
...
// ── 1. aimTime: 1.6 -> gap anchored at the player's t=1.6 position ─────────
{
  const e = mkEngine();
  const angles = fanAnglesAtFire(e, Object.assign({}, FAN_BASE, { aimTime: 1.6 }), moveLeft);
  ...
  assert(Math.abs(gap.center - Math.PI / 2) < 0.05,
    'aimTime fan gap anchored at t=1.6 player pos (center=' + gap.center.toFixed(3) + ', want ' + (Math.PI / 2).toFixed(3) + ')');
}
...
// ── 3. The two gap centers actually differ (the bug is real & fixed) ───────
{
  const e1 = mkEngine();
  const e2 = mkEngine();
  const a1 = fanAnglesAtFire(e1, Object.assign({}, FAN_BASE, { aimTime: 1.6 }), moveLeft);
  const a2 = fanAnglesAtFire(e2, Object.assign({}, FAN_BASE), moveLeft);
